...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // blokkeren als de categorie nog locaties bevat (verplaats of verwijder ze eerst)
    $telStmt = $conn->prepare("SELECT COUNT(*) AS aantal FROM tbl_locaties WHERE categorie = ?");
    ...
    if ($aantal > 0) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Deze categorie bevat nog ' . $aantal . ' locatie(s). Verplaats of verwijder die eerst.'
        ]);
        exit;
    }

    $del = $conn->prepare("DELETE FROM tbl_locatie_categorie WHERE slug = ?");
