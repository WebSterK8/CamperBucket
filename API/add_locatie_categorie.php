...
function maakLocatieSlug($naam, $conn) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $slug = maakLocatieSlug($naam, $conn);

    // positie achteraan (max + 1)
    $positie = (int) ($conn->query("SELECT COALESCE(MAX(positie), 0) + 1 AS pos FROM tbl_locatie_categorie")->fetch_assoc()['pos']);

    $sql = "INSERT INTO tbl_locatie_categorie (slug, naam, positie) VALUES (?, ?, ?)";
